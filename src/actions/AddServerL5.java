...
/**
 * the action used to represent adding a server at location 5
 * 
 * @author Zack
 * 
 */
public class AddServerL5 extends Actions {

	// the cost of adding server at location 5
	...
	/**
	 * Initializes an add Location 5 Server action
	 * 
	 * @param gpConf
	 ...
	 */
	public AddServerL5(GPConfiguration gpConf)
			throws InvalidConfigurationException {
		super(gpConf);
		this.gpConf = gpConf;
		this.timeToPeformAction = 500;
	}
	...
	/**
	 * updates a state by adding a server at location 5
	 * 
	 * @return cr - the state
	 */
	public void results(CostRewardObject cr) {
		ArrayList<Integer> usedServers = cr.getSystemState().getUsedServersL5();
		for (int i = 0; i < cr.getSystemState().validL5Servers.length; i++) {
			if (!(usedServers.contains(new Integer(
					cr.getSystemState().validL5Servers[i])))) {
				cr.getSystemState().addUsedServerL5(
						new Integer(cr.getSystemState().validL5Servers[i]));
				break;
			}
		}
	...
	/**
	 * Returns the string representation of adding a server at location 5 used
	 * when print a plan
	 * 
	 * @return the string representation of adding a server at location 5
	 */
	@Override
	...
	/**
	 * Return the prism state changes for failing to add a server at location 5
	 * 
