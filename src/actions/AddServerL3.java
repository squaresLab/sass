 ...
 * 
 */
public class AddServerL3 extends Actions {

	// the cost of adding server at location 3
	...
	/**
	 * Initializes an add Location 3 Server action
	 * 
	 * @param gpConf
	 ...
	 */
	public AddServerL3(GPConfiguration gpConf)
			throws InvalidConfigurationException {
		super(gpConf);
		this.gpConf = gpConf;
	...
	/**
	 * updates a state by adding a server at location 3
	 * 
	 * @return cr - the state
	 */
	public void results(CostRewardObject cr) {
		ArrayList<Integer> usedServers = cr.getSystemState().getUsedServersL3();
		for (int i = 0; i < cr.getSystemState().validL3Servers.length; i++) {
			if (!(usedServers.contains(new Integer(
					cr.getSystemState().validL3Servers[i])))) {
				cr.getSystemState().addUsedServerL3(
						new Integer(cr.getSystemState().validL3Servers[i]));
				break;
			}
		}
	...
	/**
	 * Returns the string representation of adding a server at location 3 used
	 * when print a plan
	 * 
	 * @return the string representation of adding a server at location 3
	 */
	@Override
