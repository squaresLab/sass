...
/**
 * the action used to represent adding a server at location 6
 * 
 * @author Zack
 * 
 */
public class AddServerL6 extends Actions {

	// the cost of adding server at location 6
	...
	/**
	 * Initializes an add Location 6 Server action
	 * 
	 * @param gpConf
	 ...
	 */
	public AddServerL6(GPConfiguration gpConf)
			throws InvalidConfigurationException {
		super(gpConf);
		this.gpConf = gpConf;
	...
	public boolean arePreconditionsSatisfied(CostRewardObject cr) {
		if (cr.getSystemState().validL6Servers.length == cr.getSystemState()
				.getUsedServersL6().size()) {
			return false;
		}
	...
	/**
	 * updates a state by adding a server at location 6
	 * 
	 * @return cr - the state
	 */
	public void results(CostRewardObject cr) {
		ArrayList<Integer> usedServers = cr.getSystemState().getUsedServersL6();
		for (int i = 0; i < cr.getSystemState().validL6Servers.length; i++) {
			if (!(usedServers.contains(new Integer(
					cr.getSystemState().validL6Servers[i])))) {
				cr.getSystemState().addUsedServerL6(
						new Integer(cr.getSystemState().validL6Servers[i]));
				break;
			}
		}
	...
	/**
	 * Returns the string representation of adding a server at location 6 used
	 * when print a plan
	 * 
	 * @return the string representation of adding a server at location 6
	 */
	@Override
	...
	/**
	 * Return the prism state changes for failing to add a server at location 6
	 * 
