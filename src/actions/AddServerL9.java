 ...
 * 
 */
public class AddServerL9 extends Actions {

	// the cost of adding server at location 9
	...
	/**
	 * Initializes an add Location 9 Server action
	 * 
	 * @param gpConf
	 ...
	 */
	public AddServerL9(GPConfiguration gpConf)
			throws InvalidConfigurationException {
		super(gpConf);
		this.gpConf = gpConf;
	...
	public boolean arePreconditionsSatisfied(CostRewardObject cr) {
		if (cr.getSystemState().validL9Servers.length == cr.getSystemState()
				.getUsedServersL9().size()) {
			return false;
		}
		if (cr.systemResponseTime < 9) {
			return false;
		}
		return true;
	}
	...
	/**
	 * updates a state by adding a server at location 9
	 * 
	 * @return cr - the state
	 */
	public void results(CostRewardObject cr) {
		ArrayList<Integer> usedServers = cr.getSystemState().getUsedServersL9();
		for (int i = 0; i < cr.getSystemState().validL9Servers.length; i++) {
			if (!(usedServers.contains(new Integer(
					cr.getSystemState().validL9Servers[i])))) {
				cr.getSystemState().addUsedServerL9(
						new Integer(cr.getSystemState().validL9Servers[i]));
				break;
			}
		}
	...
	/**
	 * Returns the string representation of adding a server at location 9 used
	 * when print a plan
	 * 
	 * @return the string representation of adding a server at location 9
	 */
	@Override
