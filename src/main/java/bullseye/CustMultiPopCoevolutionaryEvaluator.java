...
public class CustMultiPopCoevolutionaryEvaluator extends Evaluator
    {
    ...
    // number of partners from hall of champions
    public static final String P_NUM_HALL = "num-hall";
    protected int numHall;
    
    public static final String P_HALL_SIZE= "hall-size";
    protected int hallSize;
    
    HallOfChampions[/*subpop*/] hallOfChamps;
    ...
    public void setup( final EvolutionState state, final Parameter base )
        ...
        hallSize = state.parameters.getInt( base.push(P_HALL_SIZE), null, 0 );
        numHall = state.parameters.getInt( base.push(P_NUM_HALL), null, 0 );
        
        if (numHall > 0) {
            hallOfChamps = new HallOfChampions[numSubpopulations];
        	for (int i = 0; i < numSubpopulations; i++) {
	            hallOfChamps[i] = new HallOfChampions(hallSize);
        	}
        }
    ...
    protected void beforeCoevolutionaryEvaluation( final EvolutionState state, final Population population, final GroupedProblemForm prob )
        ...
        if (state.generation == 0)
            ...
            // deal with hall of champs
            if (numHall > 0) {
	            // just put in with first ind for first gen
	            for (int i = 0; i < state.population.subpops.size(); i++)
	            {
	            	hallOfChamps[i].add(state.population.subpops.get(i).individuals.get(0));
	            }
            }
            }
        }
    ...
    public void performCoevolutionaryEvaluation( final EvolutionState state,
        ...
        for(int j = 0; j < state.population.subpops.size(); j++)
            ...
            for(int i = 0; i < state.population.subpops.get(j).individuals.size(); i++)
                ...
                // test against individuals in the hall of champs
                for(int k = 0; k < numHall; k++)
                    {
                    for(int ind = 0; ind < inds.length; ind++)
                        {
                        if (ind == j) { inds[ind] = individual; updates[ind] = true; }
                        else { inds[ind] = hallOfChamps[ind].getRandInd(); updates[ind] = true; }
                        }
                    prob.evaluate(state,inds,updates, false, subpops, 0);
                    evaluations++;
                    }
        ...
        if (numHall > 0) {
            for(int i = 0; i < state.population.subpops.size(); i++) {
	            ...
	            for(int x=1;x<oldinds.size();x++)
	                if (oldinds.get(x).fitness.betterThan(best.fitness))
	                    best = oldinds.get(x);
	            hallOfChamps[i].add(best);
	            }
       }
