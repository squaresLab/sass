...
public class Lexer implements LexerConstants {
		...
		if (e.getClass().equals(TokenMgrError.class)) {
			...
			for (i = 0; i < tmp.length; i++) {
				if (tmp[i].equals("line") && i + 1 < tmp.length) {
					l = Integer
							.parseInt(tmp[i + 1].substring(0, tmp[i + 1].length() - 1));
				}
	...
	private void skipToNextBlock() {
		...
		do {
			...
			t2 = getToken(2);
				...
				&& t2.kind != PREDICATES && t2.kind != TYPES && t2.kind != CONSTANTS
				&& t2.kind != FUNCTIONS && t2.kind != ACTION
				&& t2.kind != DURATIVE_ACTION && t2.kind != DERIVED
				&& t2.kind != OBJECTS && t2.kind != INIT && t2.kind != GOAL
				&& t2.kind != METRIC && t2.kind != EOF);
	}
		...
		try {
			...
			if (jj_2_5(2)
					&& (this.isRequirement(RequireKey.FLUENTS)
							...
							|| this.isRequirement(RequireKey.OBJECT_FLUENTS) || this
								.isRequirement(RequireKey.ACTION_COSTS))) {
				functions_def();
			} else {
				;
			}
	...
	final public List<Symbol> function_type() throws ParseException {
		...
		if (jj_2_15(2)
				&& (this.isRequirement(RequireKey.NUMERIC_FLUENTS)
						|| this.isRequirement(RequireKey.FLUENTS) || this
							.isRequirement(RequireKey.ACTION_COSTS))) {
			...
			typesList.add(new Symbol(Symbol.Kind.TYPE, token));
			{
				...
				&& (this.isRequirement(RequireKey.OBJECT_FLUENTS) || this
						.isRequirement(RequireKey.FLUENTS))) {
	...
	final public Exp node() throws ParseException {
		...
		if (jj_2_31(2)) {
			...
			node = not_gd();
				...
				&& (this.isRequirement(RequireKey.DISJUNCTIVE_PRECONDITIONS) && this
			...
			node = equal_gd();
				...
				&& (this.isRequirement(RequireKey.FLUENTS)
						|| this.isRequirement(RequireKey.NUMERIC_FLUENTS) || this
							.isRequirement(RequireKey.OBJECT_FLUENTS))) {
			node = f_comp();
		} else {
			jj_consume_token(-1);
			throw new ParseException();
	...
	/*
	 * Exp function_term() : { Exp node = new Exp(Connective.F_EXP); List<Symbol>
	 * function = new ArrayList<Symbol>(); Symbol t = null; } { <LEFT_BRACKET>
	 * t=functor() { function.add(t); } (t=constant() | t=variable()) {
	 * function.add(t); })* right_bracket() { node.setAtom() return node; } }
	 */
				...
				if (!jj_semLA || jj_3_33()) {
					...
					if (!jj_semLA || jj_3_34()) {
						...
						if (!jj_semLA || jj_3_35()) {
							...
							lookingAhead = true;
							jj_semLA = this
									.isRequirement(RequireKey.EXISTENTIAL_PRECONDITIONS);
							lookingAhead = false;
							if (!jj_semLA || jj_3_36()) {
								jj_scanpos = xsp;
								lookingAhead = true;
	...
	private static void jj_la1_0() {
		jj_la1_0 = new int[] { 0x0, 0x0, 0x0, 0x0, 0x0, 0x2000, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x2000, 0x0, 0x2000, 0x0, 0x0, 0x2000, };
	}

	private static void jj_la1_1() {
		jj_la1_1 = new int[] { 0x1fffff80, 0x1fffff80, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, };
	}

	private static void jj_la1_2() {
		jj_la1_2 = new int[] { 0x0, 0x0, 0x0, 0x3c000000, 0x0, 0x0, 0x0,
				0x3c000000, 0x3c000000, 0x0, 0x20, 0x3c000000, 0x3c000000, 0x3c000000,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80000000, };
	}

	private static void jj_la1_3() {
		jj_la1_3 = new int[] { 0x0, 0x0, 0x2c0, 0x200, 0x100, 0x2c0, 0x2c0, 0x200,
				0x200, 0x201, 0x0, 0x200, 0x300, 0x300, 0x400, 0x201, 0x200, 0x400,
				0x200, 0x200, 0x400, };
	}
