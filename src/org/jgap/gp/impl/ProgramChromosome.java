 ...
 * @since 3.0
 */
public class ProgramChromosome extends BaseGPChromosome implements Comparable,
		Cloneable, IBusinessKey {
	...
	private transient static Logger LOGGER = Logger
			.getLogger(ProgramChromosome.class);
	...
	public ProgramChromosome(GPConfiguration a_conf, int a_size, IGPProgram a_ind)
			throws InvalidConfigurationException {
		super(a_conf, a_ind);
		if (a_size <= 0) {
			throw new IllegalArgumentException(
	...
	public ProgramChromosome(GPConfiguration a_conf, int a_size,
			CommandGene[] a_functionSet, Class[] a_argTypes, IGPProgram a_ind)
			throws InvalidConfigurationException {
		super(a_conf, a_ind);
		if (a_size <= 0) {
			throw new IllegalArgumentException(
	...
	private void init(final int a_size) throws InvalidConfigurationException {
	...
	public synchronized Object clone() {
		try {
			...
			ProgramChromosome chrom = new ProgramChromosome(
					(GPConfiguration) getGPConfiguration(), (CommandGene[]) genes);
	 ...
	 * 
	 * @param a_num
	 *          the chromosome's index in the individual of this chromosome
	 * @param a_depth
	 *          the maximum depth of the chromosome to create
	 * @param a_type
	 *          the type of the chromosome to create
	 * @param a_argTypes
	 *          the array of argument types for this chromosome
	 * @param a_functionSet
	 *          the set of nodes valid to pick from
	 * @param a_grow
	 *          true: use grow method; false: use full method
	 * @param a_tries
	 *          maximum number of tries for creating a valid program
	 * 
	 * @author Klaus Meffert
	 * @since 3.0
	 */
	public void growOrFull(final int a_num, final int a_depth,
			final Class a_type, final Class[] a_argTypes,
			final CommandGene[] a_functionSet, boolean a_grow, int a_tries) {
		try {
			argTypes = a_argTypes;
			...
			for (int i = 0; i < a_argTypes.length; i++) {
				m_functionSet[a_functionSet.length + i] = new Argument(
						getGPConfiguration(), i, a_argTypes[i]);
			}
			...
			// ------------------------------------------------
			if (!getGPConfiguration().validateNode(this, null, n, a_tries, a_num, 0,
					a_type, m_functionSet, a_depth, a_grow, -1, true)) {
	 ...
	 * 
	 * @param a_startNode
	 *          the node to start with, e.g. 0 for a complete dump of the program
	...
	public String toStringNorm(final int a_startNode) {
		...
		for (int i = 0; i < m_genes[a_startNode].getArity(ind); i++) {
			...
			if (i == 0 && m_genes[a_startNode].getArity(ind) != 1 && !paramOutput) {
	 ...
	 * 
	 * @param a_returnType
	 *          the return type to look for
	 * @param a_subReturnType
	 *          the sub return type to look for
	 * @param a_nodeSet
	 *          the array of nodes to look through
	 * @param a_function
	 *          true to look for a function, false to look for a terminal
	 * @param a_growing
	 *          true: grow mode, false: full mode
	 * 
	...
	public boolean isPossible(Class a_returnType, int a_subReturnType,
			CommandGene[] a_nodeSet, boolean a_function, boolean a_growing) {
		...
		for (int i = 0; i < a_nodeSet.length; i++) {
			if (a_nodeSet[i].getReturnType() == a_returnType
					&& (a_subReturnType == 0 || a_subReturnType == a_nodeSet[i]
							.getSubReturnType())) {
	 ...
	 * 
	 * @param a_chromIndex
	 *          index of the chromosome in the individual (0..n-1)
	 * @param a_returnType
	 *          the return type of node to choose
	 * @param a_subReturnType
	 *          the sub return type to look for
	 * @param a_functionSet
	 *          the functions to use
	 * @param a_function
	 *          true to choose a function, false to choose a terminal
	 * @param a_growing
	 *          true to ignore the function parameter, false otherwise
	...
	protected CommandGene selectNode(int a_chromIndex, Class a_returnType,
			int a_subReturnType, CommandGene[] a_functionSet, boolean a_function,
			boolean a_growing) {
		...
		for (int i = 0; i < a_functionSet.length; i++) {
			if (a_functionSet[i].getReturnType() == a_returnType
					&& (a_subReturnType == 0 || a_subReturnType == a_functionSet[i]
							.getSubReturnType())) {
				if (a_functionSet[i].getArity(ind) == 0 && (!a_function || a_growing)) {
					...
					if (singleNodeValidator == null
							|| singleNodeValidator.isAllowed(a_chromIndex, this,
									a_functionSet, a_functionSet[i], a_returnType,
									a_subReturnType, m_index)) {
		...
		if (possibleFunctions.isEmpty()) {
			if (a_growing
					&& (a_returnType == CommandGene.VoidClass || a_returnType == Void.class)) {
			...
			final String errormsg = "Chromosome (depth "
					...
					+ a_chromIndex
					+ ") requires a "
					+ (a_function ? ("function" + (a_growing ? " or terminal" : ""))
							: "terminal") + " of return type " + a_returnType
	 ...
	 * @param a_returnType
	 *          the return type the lastly evaluated node must have
	 * @param a_subReturnType
	 *          the sub return type to look for
	 * @param a_functionSet
	 *          the set of function valid to pick from
	 * @param a_rootNode
	 *          null, or parent node of the node to develop
	 * @param a_recurseLevel
	 *          0 for first call
	 * @param a_grow
	 *          true: use grow method; false: use full method
	 * @param a_childNum
	 *          index of the child in the parent node to which it belongs (-1 if
	 *          node is root node)
	 * @param a_validateNode
	 *          true: check if node selected is valid (when called recursively
	 *          a_validateNode is set to true)
	 * 
	...
	protected CommandGene[] growOrFullNode(int a_num, int a_depth,
			...
			CommandGene a_rootNode, int a_recurseLevel, boolean a_grow,
		...
		if (a_rootNode == null || a_validateNode) {
			...
			do {
				...
				if (!conf.validateNode(this, node, a_rootNode, tries++, a_num,
						a_recurseLevel, a_returnType, localFunctionSet, a_depth, a_grow,
						a_childNum, false)) {
		...
		if (a_depth >= 1) {
			...
			for (int i = 0; i < arity; i++) {
				...
				if (m_index < m_depth.length) {
					a_functionSet = growOrFullNode(a_num, a_depth - 1,
							...
							a_rootNode.getSubChildType(i), a_functionSet, a_rootNode,
							a_recurseLevel + 1, a_grow, i, true);
				} else {
					...
					throw new IllegalStateException("Randomly created program violates"
							...
							+ " (current arity: " + i + ", overall arity: " + arity + ")!");
				}
			}
		} else {
			if (a_rootNode.getArity(getIndividual()) > 0) {
				...
				// -------------------------------------------
				throw new IllegalStateException("Randomly created program violates"
						+ " configuration constraints" + " (symptom 2):" + " Root node: "
	...
	/**
	 * Calculate the depth of the next node and the indices of the children of the
	 * current node. The depth of the next node is just one plus the depth of the
	 * current node. The index of the first child is always the next node. The
	 * index of the second child is found by recursively calling this method on
	 * the tree starting with the first child.
	 * 
	 * @param a_index
	 *          the index of the reference depth
	 ...
	 * 
	 * @param a_index
	 *          the node number of the parent
	 * @param a_child
	 *          the child number (starting from 0) of the parent
	 ...
	 * 
	 * @param a_index
	 *          the index of the node at which to start counting
	 ...
	 * 
	 * @param a_index
	 *          the index of the node at which to check the depth
	 ...
	 * @param a_exactMatch
	 *          true: look for exactly the given type: false: also look for sub
	 *          types
	 ...
	 *           if the type of this node is not boolean
	 * 
	 * @author Klaus Meffert
	 * @since 3.0
	 */
	 ...
	 * @param child
	 *          the child number of the node to execute
	 * @param args
	 *          the arguments for execution
	 * @return the boolean return value of this node
	 * @throws UnsupportedOperationException
	 ...
	 * @throws UnsupportedOperationException
	 *           if this node's type is not double
	 * 
	 * @author Klaus Meffert
	 * @since 3.0
	 */
	 ...
	 * @throws UnsupportedOperationException
	 *           if the type of this node is not of type Object
	 * 
	 * @author Klaus Meffert
	 * @since 3.0
	 */
	 ...
	 * @param args
	 *          the arguments for execution
	 * @return the Object which wraps the return value of this node, or null if
	 *         the return type is null or unknown
	 * 
	 * @author Klaus Meffert
	 * @since 3.0
	 */
	 ...
	 * applications when it comes time to evaluate this Chromosome in the fitness
	 * function. JGAP ignores this data functionally.
	 * 
	 ...
	 * first gene is at index zero and the last gene is at the index equal to the
	 * size of this Chromosome - 1.
	 * 
	 * @param a_locus
	 ...
	 * 
	 * @param a_geneClassName
	 *          name of the gene class
	 * @param a_persistentRepresentation
	 *          persistent representation of the gene to create (could be obtained
	...
	protected CommandGene createGene(String a_geneClassName,
		...
		Constructor constr = geneClass
				.getConstructor(new Class[] { GPConfiguration.class });
		CommandGene gene = (CommandGene) constr
				.newInstance(new Object[] { getGPConfiguration() });
	...
	protected static final List split(String a_string)
		...
		StringTokenizer st = new StringTokenizer(a_string, GENE_DELIMITER_HEADING
				+ GENE_DELIMITER_CLOSING, true);
	 ...
	 * 
	 * @param a_functionSet
	 *          the set of functions to check
	 * @param a_function
	 *          the commandgene to check for via its class
	 ...
	 *          the commandgene to check for via its class
	 * @param a_limit
	 *          if a_limit occurrences of a_function were detected, exit
	 ...
	 * @since 3.6
	 */
	public int contains(CommandGene[] a_functionSet, CommandGene a_function,
			int a_limit) {
		Class clazz = a_function.getClass();
		if (a_limit <= 0) {
	...
	/**
	 * Print valid plan size using the toString transversal metho
	 * 
	 * @param a_startNode
	 *          node to start with
	 * @return output in left-hand notion
	 ...
	 * @since 3.0
	 */
	public int validPlanSize(final int a_startNode) {
		if (a_startNode < 0) {
			return 0;
		}
		IGPProgram ind = getIndividual();
		...
		int sizeWithChildren = 1;

		int arity = m_genes[a_startNode].getArity(ind);
		for (int i = 0; i < arity; i++) {
			sizeWithChildren += validPlanSize(getChild(a_startNode, i));
		}
